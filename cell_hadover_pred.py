...
for i in range(len(drives)):
    drives_by_modem.append([v for k, v in drives[i].groupby('imei')])
...
for j in range(len(drives_by_modem[DRIVE_NUM])):  # len(drives_by_modem[i]) - number os modems in drive.
    key_for_dict = str(
        drives_by_modem[DRIVE_NUM][j]['date'].iloc[0] + '_' + drives_by_modem[DRIVE_NUM][j]['time'].iloc[0] + '_' +
        str(drives_by_modem[DRIVE_NUM][j]['imei'].iloc[0]))
    drives_by_imei_dict[key_for_dict] = drives_by_modem[DRIVE_NUM][j]
...
for key in drives_by_imei_dict.keys():  # len(drives_by_modem)
    cells_per_drive_per_modem_avg[key] = pd.concat(get_unique_cells_in_drive(drives_by_imei_dict[key]), axis=0,
                                                   join='outer').dropna(axis=0)

for key in drives_by_imei_dict.keys():
    drives_by_imei_dict[key].insert(18, "switchover", 0, allow_duplicates=False)
    drives_by_imei_dict[key].insert(18, "globacellid_shift", 0, allow_duplicates=False)
    drives_by_imei_dict[key]['globacellid_shift'] = drives_by_imei_dict[key]['globalcellid'].shift()
    drives_by_imei_dict[key]['globalcellid'] = drives_by_imei_dict[key].apply(
        lambda x: x['globacellid_shift'] if x['globalcellid'] == 0 else x['globalcellid'], axis=1)
    cell_calculation.calculate_switchover(drives_by_imei_dict[key])  # Calculate switch over per drive per imei
...
for key in drives_by_imei_dict.keys():
    ...
    drives_by_imei_dict[key]['latitude_perimeter_shift'] = drives_by_imei_dict[key]['latitude_perimeter'].shift()
    drives_by_imei_dict[key]['longitude_perimeter_shift'] = drives_by_imei_dict[key]['longitude_perimeter'].shift()
    # Calculate the Euclidean distance between the two points for each row
    drives_by_imei_dict[key]['distance'] = drives_by_imei_dict[key].apply(euclidean_distance, axis=1)
    drives_by_imei_dict[key]['speed'] = drives_by_imei_dict[key]['distance'] * 60 * 60 * 111 * -1
    drives_by_imei_dict[key].fillna(0, inplace=True)
    ...
    data1 = drives_by_imei_dict[key][['latitude_perimeter', 'longitude_perimeter', 'speed']]
    ...
    data2 = drives_by_imei_dict[key][['latitude_perimeter', 'longitude_perimeter', 'switchover']]
    ...
    data_filter2 = data2.values.tolist()
    HeatMap(data=data_filter2, use_local_extrema=False, min_opacity=0.0, max_opacity=0.95, radius=10).add_to(
        folium.FeatureGroup(name=str(key) + ' switchover').add_to(map_switchover_for_drive_for_modem))

    data3 = drives_by_imei_dict[key][['latitude_perimeter', 'longitude_perimeter', 'loss_rate']]
    ...
    data4 = drives_by_imei_dict[key][['latitude_perimeter', 'longitude_perimeter', 'rsrp']]
